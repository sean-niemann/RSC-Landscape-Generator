...
public class BZip {
    ...
    private static void nextHeader(BZipState state) {
    ...
    private static void decompress(BZipState state) {
        int gMinLen = 0;
        int gLimit[] = null;
        int gBase[] = null;
        int gPerm[] = null;
    ...
    private static byte getUchar(BZipState state) {
        return (byte) getBits(8, state);
    }

    private static byte getBit(BZipState state) {
        return (byte) getBits(1, state);
    }

    private static int getBits(int i, BZipState state) {
        int vvv;
        do {
            if (state.bsLive >= i) {
    ...
    private static void makeMaps(BZipState state) {
        state.nInUse = 0;
        for (int i = 0; i < 256; i++)
            if (state.inUse[i]) {
